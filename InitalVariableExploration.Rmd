...
train_houses%>%filter(X2ndFlrSF!=0)%>%ggplot(aes(x=X2ndFlrSF,y=log(SalePrice)))+geom_point()

X2ndFlrSFFit = lm(SalePrice~X2ndFlrSF, train_houses)

train_houses%>%add_residuals(X2ndFlrSFFit)%>%
  ggplot(aes(x=X2ndFlrSF,y=resid))+geom_point(alpha=0.5)
rmse(X2ndFlrSFFit, train_houses)
```
...
I wiped out the 0 value of the saleprice while ploting the relationship between X2ndFlrSF and SalePrice, because if the 0 value means the house doesn't have a second floor, so the salevalue doesn' infected by the 0 value. And there is a positive linar relationship of X2ndFlrSF and log(salePrice). There is clear positive relationship between X2ndFlrSF and residuals. So I won't add the variable into the model.
##LowQualFinSF
```{r}
...
train_houses%>%filter(LowQualFinSF > 0)%>%ggplot(aes(x=LowQualFinSF,y=SalePrice))+geom_point()

```
Heavily skewed left, mean median and mode are 0 SF. No missing values. A lot of outliers. When all the 0 values are taken away, the distribution is slightly skewed right with no outliers.I filtered the variable more than 0, because 0 value means there is no Low quality finished square in the house and thus doesn't have any infect on the sale price. There is no clear relationship of LowQualFinSF of SalePrice. So I won't add this into the model.
##GrLivArea
```{r}
...
train_houses%>%ggplot(aes(x=GrLivArea,y=SalePrice))+geom_point()+geom_smooth()
train_houses%>%ggplot(aes(x=PropGrLivArea,y=SalePrice))+geom_point()+geom_smooth()

GrLivAreaFit=lm(SalePrice~GrLivArea,train_houses)

train_houses%>%add_residuals(GrLivAreaFit)%>%ggplot(aes(x=GrLivArea,y=resid))+geom_point(alpha=0.5)
rmse(GrLivAreaFit, train_houses)
```
...
Because as there is more SF the sale price will be higher. So I will chose the GrLivArea to do analysis. As it's shown in the point plot, there is clear positive relationship. And the residuals are symmatric distributed. So I will consider add it into the model. And the rmse is low, so it may be appropriate to add into the model.
##BsmtFullBath
```{r}
...
##transfor into factors
train_houses <- train_houses%>%mutate(FBsmtFullBath = factor(BsmtFullBath, level=c(0,1,2,3)))
train_houses%>%ggplot()+geom_boxplot(aes(x=FBsmtFullBath,y=SalePrice))


```
Left skewed, majority have 0 full bathrooms in the basement when looking at all observations. Since there are only at most 37 observations in the data that do not have a basement filtering for those will not produce significant changes. The sale price increased between 0 and 1 Basement full bathrooms. But there is not too much different among 1, 2 and 3 full bathrooms. So I think there is oly weak realtionship, I won't consider using the variable while doing the model. 
##BsmtHalfBath
```{r}
summary(train_houses$BsmtHalfBath)

train_houses %>%filter(!is.na(BsmtHalfBath))%>%
  ggplot()+geom_bar(aes(x=BsmtHalfBath))

##transfor into factors
train_houses <- train_houses%>%mutate(FBsmtHalfBath=factor(BsmtHalfBath,level=c(0,1)))


train_houses%>%filter(!is.na(BsmtHalfBath))%>%ggplot(aes(x=FBsmtHalfBath,y=SalePrice))+geom_boxplot()

```
Left skewed. Mode is 0. There is no clear relationship between Basement half bathrooms and sale price, so I won't use this value.
...
train_houses%>%ggplot(aes(x=factor(TotalBath,levels = c(0,0.5,1,1.5,2,2.5,3,3.5,4,4.5,5,5.5,6)),y=SalePrice))+geom_boxplot()

TotalBathFit=lm(SalePrice~TotalBath, train_houses)

train_houses%>%add_residuals(TotalBathFit)%>%ggplot(aes(x=factor(TotalBath,levels = c(0,0.5,1,1.5,2,2.5,3,3.5,4,4.5,5,5.5,6)), y=resid))+geom_boxplot()
...
There is a positive relationship while the number of TotalBath less than 4.5. But the relationship is not strong, plus there is no positive relationship while x range in [4.5,6]. And the residuals are not random distributed. So I don't think this is a good to add into the model.
##BedroomAbvGr
```{r}
...
train_houses%>%ggplot(aes(x=factor(BedroomAbvGr),y=SalePrice))+geom_boxplot()
```
...
There is no clear relationship between BedroomAbvGr anf SalePrice.
##KitchenAbvGr
```{r}
...
train_houses%>%ggplot(aes(x=factor(KitchenAbvGr),y=SalePrice))+geom_boxplot()
```
...
There is not clear relationship between KitchenAbvGr. It seems like the 1 kitchen house has higher price. But the relationship is not strong.
##KitchenQual
```{r}
...
```
```{r}
train_houses%>%filter(!is.na(KitchenQual))%>%ggplot(aes(x=factor(KitchenQual),y=SalePrice))+geom_boxplot()

KitchenQualFit =lm(SalePrice~KitchenQual,train_houses)

train_houses%>%filter(!is.na(KitchenQual))%>%add_residuals(KitchenQualFit)%>%ggplot(aes(x=factor(KitchenQual),y=resid))+geom_boxplot()
rmse(KitchenQualFit, train_houses)
```
...
The relationship between KitchenQual is a clear positive relationship, the better kitchen quality the higher the price of the house. But the residuals are not randomly distributed, so it may not be good enough to add into the model.
##TotRmsAbvGrd
```{r}
...
train_houses = train_houses%>%mutate(TotRooms = TotRmsAbvGrd+TotalBath)


train_houses%>%ggplot(aes(x=factor(TotRmsAbvGrd),y=SalePrice))+geom_boxplot()
train_houses%>%ggplot(aes(x=factor(TotRooms),y=SalePrice))+geom_boxplot()

TotRmsAbvGrdFit=lm(SalePrice~TotRmsAbvGrd, train_houses)
train_houses%>%add_residuals(TotRmsAbvGrdFit)%>%ggplot(aes(x=factor(TotRmsAbvGrd),y=resid))+geom_boxplot()

rmse(TotRmsAbvGrdFit, train_houses)
```
...
There is positive relationship between Total rooms above grade (does not include bathrooms) and sale price ,and there is no clear relationship when included bathrooms. And the residuals are randomsly distributed. There is except case when the variable is 12 and 14, so I will reconsider if this one is good to be add into model.
##Functional
```{r}
...
train_houses%>%ggplot(aes(x=Functional, y=SalePrice))+geom_boxplot()
```
...
There is no relationship between Functional and SalePrice.
##Fireplaces
```{r}
...
train_houses%>%ggplot(aes(x=factor(Fireplaces),y=SalePrice))+geom_boxplot()

FirePlaceFit = lm(SalePrice~Fireplaces, train_houses)

train_houses%>%ggplot(aes(x=factor(Fireplaces),y=SalePrice))+geom_boxplot()
```
No missing values, data is left skewed, any properties with more than 2 fireplaces is an outlier.
There is positibe relasionship between Fireplace and SalePrice. The residuals have a clear relationship with Fireplaces, so it's not appropriate to be added into the model.
##FireplacesQu
```{r}
...
train_houses%>%filter(!is.na(FireplaceQu))%>%ggplot(aes(x=FireplaceQu, y=SalePrice))+geom_boxplot()
FireplaceQuFit = lm(SalePrice~FireplaceQu, train_houses)

train_houses%>%filter(!is.na(FireplaceQu))%>%add_residuals(FireplaceQuFit)%>%ggplot(aes(x=FireplaceQu, y=SalePrice))+geom_boxplot()
...
There is very clear positive relationship between FireplaceQu. But there is clear relationship with residuals, as shown in the plot, the residuals has a positive relationship against FirePlace. So this variable is not appropriate to be added into the model.
##GarageType
```{r}
train_houses$GarageType <- factor(train_houses$GarageType, levels = c('Detachd','CarPort','BuiltIn','Basement','Attchd', '2Types','NA'))
...
train_houses%>%ggplot(aes(x=GarageType, y=SalePrice))+geom_boxplot()

GarageTypeFit = lm(SalePrice~GarageType, train_houses)

train_houses%>%add_residuals(GarageTypeFit)%>%ggplot(aes(x=GarageType, y=resid))+geom_boxplot()
```
...
There is some relationship between GarageType and SalePrice, but the residuals are not symmetricly distributed. When the GarageType is Attchd the residuals are lower than other case.
##GarageFinish
```{r}
